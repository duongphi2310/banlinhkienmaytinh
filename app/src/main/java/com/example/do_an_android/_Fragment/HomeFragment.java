...
public class HomeFragment extends Fragment implements View.OnClickListener {
    ...
    // -- TỰ ĐỘNG CUỘN ẢNH TRÊN SLIDER. -- (DONE)
    ...
    private void addDataDiscounted() {
        ...
        RequestQueue queue = Volley.newRequestQueue(context);
        JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(Server.urlProductsDiscounted, response -> {
            for (int i = 0; i < response.length(); i++) {
                try {
                    JSONObject jsonObject = response.getJSONObject(i);
    ...
    private void setCategoryRecycler() {
        ...
        categoryRecyclerView.addItemDecoration(new GridSpacingItemDecoration(categoryList.size()-3,dpToPx(-1),false));
