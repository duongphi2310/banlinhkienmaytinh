...
public class UpdateCustomerFragment extends Fragment implements View.OnClickListener {
    ...
    EditText name_updateCustomer, email_updateCustomer, address_updateCustomer, phone_updateCustomer, passOld, passNew, passNewConfirm;
    Button btnUpdateCustomer, btnChangepass, btnConfirmChangpass, btnCancelChangpass, btnManage;
    ...
    private void getDataCustomer() {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Server.urlGetCustomerByUsername + "?username=" + username, null, response -> {
            try {
                ...
                email_updateCustomer.setText(response.getString("email"));
                address_updateCustomer.setText(response.getString("address"));
                phone_updateCustomer.setText(0 + response.getString("phone"));

                int idquyen = response.getInt("idquyen"); // Lấy giá trị idquyen từ response

                if (idquyen == 1) {
                    // Nếu quyền là 1 thì hiển thị nút quản lý và điều hướng đến TrangQuanLy
    ...
    private void setControl(View view) {
        ...
        email_updateCustomer = view.findViewById(R.id.email_updateCustomer);
        email_updateCustomer.setFocusable(false);
        email_updateCustomer.setFocusableInTouchMode(false);
        email_updateCustomer.setClickable(false);
    ...
    private void changePass() {
        StringRequest stringRequest = new StringRequest(Request.Method.POST, Server.urlChangePassCustomer, response -> {
    ...
    private void updateInfoCustomer() {
        if (validateEditText(name_updateCustomer, "Tên khách hàng không được để trống.")
                || validateEditText(email_updateCustomer, "Email không được để trống."))
            return;
    ...
    private void updateTextCustomer() {
        StringRequest stringRequest = new StringRequest(Request.Method.POST, Server.urlUpdateCustomer, response -> {
            ...
            protected Map<String, String> getParams() {
                ...
                params.put("email", email_updateCustomer.getText().toString());
